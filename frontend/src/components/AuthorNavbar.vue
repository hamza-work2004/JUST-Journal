...
// 1. تعديل دالة فتح القائمة (شطبنا منها تصفير العداد)
const toggleNotifications = () => {
    showNotifDropdown.value = !showNotifDropdown.value;
    showProfileDropdown.value = false;
};

// 🔥 2. دالة جديدة: قراءة إشعار واحد فقط عند التمرير
const markOneAsRead = async (notification) => {
    // إذا كان أصلاً مقروء، ما تعمل اشي
    if (notification.is_read === 1) return;

    // تحديث محلي فوري (عشان السرعة في العرض)
    notification.is_read = 1;
    if (unreadCount.value > 0) {
        unreadCount.value--;
    }

    // (اختياري) هنا بتقدر تبعث طلب للباك إند عشان يحفظ إن هذا الإشعار انقرأ
    /*
    try {
        await axios.put(`${API_BASE_URL}/markNotificationRead/${notification.id}`);
...
<style scoped>
/* نفس الستايل بالضبط ما تغير عليه اشي */
